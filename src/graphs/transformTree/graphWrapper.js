...
//   actualTwoD4,
// } from "../utils/mockData";
import TransformTreeGraph from './transformTree.graph';
...
const TransformTree = (props) => {
  ...
  // useEffect(() => {
  //   graph = graph || new TransformTreeGraph(refElement.current);
  //   graph.setData(data);
  ...
  return (
    <GraphContainer className="transform-tree graph-container">
      <GraphWrp className="graph-wrp" ref={refElement}></GraphWrp>
    </GraphContainer>
  );
};

TransformTree.propTypes = {
  data: PropTypes.object,
  config: PropTypes.object,
  rerender: PropTypes.bool,
};

export default ResizeHandlerHOC(TransformTree);
